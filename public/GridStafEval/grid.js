...
function BindingZeroClipboard(id, tablename) {
	...
	clip.addEventListener('mousedown', function () {
		...
		$('#' + tablename + ' tr').each(function () {
			$(this).find('td').each(function () { var cellinnerHTML = $(this).text(); copytxt = copytxt + cellinnerHTML + '\t'; })
			copytxt = copytxt + "\n";
		}) //copytxt=document.getElementById('tableContent').innerHTML;
		clip.setText(copytxt);
	});
	clip.addEventListener('complete', function (client, text) { alert('copied:'); });
	clip.glue(id);
}
function bind_txtinput_paste($txtinput, cell) {
	$txtinput.bind('paste', function (e) {
		...
		for (j = 0; j < rowcnt; j++) {
			...
			if (temp_ar.length >= colcnt) { for (k = 0; k < colcnt; k++) f_data_arr[j][k] = temp_ar[k]; }
		}
		if (rowcnt > 1 || colcnt > 1) {
			if (confirm("Row x Col :" + rowcnt + "x" + colcnt + " past to " + rowcnt + "x" + colcnt + " ? ")) {
				var c_cell = cell;
				var c_cell_col_id = 0;
				var f_cell = c_cell.parent().children('td:first');
				for (i = 0; i < 20; i++) {
					c_cell_col_id++;
					if (c_cell.attr('id') == f_cell.attr('id')) {	break;}
					f_cell = f_cell.next();
				}
...
function GenOriginalData() {
	if (OriginalData == null) {
		closeedit();
		OriginalData = {};
		$('.M').each(function (i) {	OriginalData[$(this).attr('id')] = $(this).text(); });
...
function SplitPastFrmText(txt) {
	...
	var colcnt = data_arr[0].split('\t').length;
	var f_data_arr = new Array();
	for (i = 0; i < colcnt; i++) {	f_data_arr[i] = new Array();}
	for (j = 0; j < rowcnt; j++) {
		var temp_ar = data_arr[j].split('\t');
		if (temp_ar.length >= colcnt) {
			for (k = 0; k < colcnt; k++)	f_data_arr[k][j] = temp_ar[k];
		}
	}
	return f_data_arr;
}
...
function BindingPastFrm(frm, txt, table) {
	...
	$("#" + frm).dialog(
		{
			autoOpen: false,	minWidth: 500,	title: 'Past:',
			buttons: {
				"past": function () {
					closeedit();
...
function BindingCSVFrm(frm, linkid, tabletxt, filename) {
	...
	$('#' + frm).dialog(
		{
			autoOpen: false,	minWidth: 500,	title: 'CSV:',
			open: function () {				
				closeedit();
		...
		if (PostUrl != null) {
			$.post(PostUrl, { datajson: JSON.stringify(json), keycode: '125678985432' })
				.done(function (data) {
					...
					for (var key in json) OriginalData[key] = "-1";
				});
		} else {
...
$(document).ready(function () {
	$('td.M').click(function (event) { let obj = editCell($(this)); if (obj) obj.focus(); });
	function EndModify() {
		$('.M').each(function (i) {
			if ($(this).has(":input").length == 0) {
			} else {
	...
	$("td.M").keydown(function (e) {
		if (e.keyCode == 38 || e.keyCode == 40 || e.keyCode == 13) {
			...
			$(':input').each(function (i) {
				//if($(this).has(":input").length!=0)
				//{
				cells[count] = $(this).parent();
				if ($(this).parent().attr('id') == cell_id) rowid = count;
				count++;
				//}
			});
			switch (e.keyCode) {
