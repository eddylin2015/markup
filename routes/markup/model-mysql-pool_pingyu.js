...
async function UpdatePingYu( aot, aObj, cb) {
        ...
        if (aObj) {
            ...
            for (let i = 0; i < alist.length; i++) {
                ...
                cnt += await new Promise((resolve, reject) => {
                    connection.query(`update mrs_pingyu set ${fieldname}=? where lineno=0 and stud_ref=?`, [pyStmt,studref], (err, res) => {
...
async function UpdatePingYuArr( aot, alist, cb) {
    pool.getConnection(async function (err, connection) {
        if (err) { cb(err); return; }
        let cnt = 0;
        if (alist) {
